...
def main():
    ...
    ticker_data = safe_call("ticker", missing_data, client.ticker, symbol)
    ticker = ticker_data[0] if ticker_data else {}

    timeframes = {}
    for tf, okx_bar in collection["timeframes"].items():
        candles = safe_call(f"{tf} candles", missing_data, client.candles, symbol, okx_bar, collection["candle_limit"])
        if candles is None:
            timeframes[tf] = {"candles": [], "indicators": {}}
            continue
        timeframes[tf] = {"candles": candles, "indicators": calculate_timeframe_indicators(candles)}

    book = safe_call("order book", missing_data, client.order_book, symbol, collection["order_book_depth"])
    trades = safe_call("recent trades", missing_data, client.trades, symbol, collection["trades_limit"])
    funding_data = safe_call("funding rate", missing_data, client.funding_rate, symbol)
    oi_data = safe_call("open interest", missing_data, client.open_interest, symbol)

    account = {}
    positions = []
    if client.has_credentials:
        account = normalize_account(safe_call("account balance", missing_data, client.account_balance) or [])
        positions = safe_call("positions", missing_data, client.positions, symbol) or []
    else:
